...
|---|---|---|---|
| dual (control) | `/gig/` | `dual-c` (`dual` until 2026-09-23, then briefly `dual-b`) | both sides (broad/organic) |
...
- **`/gig/` is the ad page (`dual-c`).** At the owner's direction (2026-09-23) it is a scoped exception to design rules 1–4: a blue brand band on the closing section (the app icon's cobalt), green kept only for money figures, big stat blocks, a shadowed phone. The hero is the original `dual` hero (light page, metal headline, couplet, one phone) — `dual-b` was the same page with a cobalt-band, two-phone hero, live for a few hours on 2026-09-23. It lives entirely in `styles/gig-ad.css` under `html[data-variant="dual-c"]` and must not leak to other arms or the hub. AA, mobile, copy honesty and the CTA rules still apply in full. The "~$15+ in fees" money block uses the same hedged claim as `/gig/compare/`, so keep the two in step.
